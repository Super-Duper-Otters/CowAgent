...
def _investment_customer_snapshot(user):
    if user is None:
        return {}
    return {
        "openid": user.openid,
...
def _record_investment_operation(
    action: str,
    target_type: str,
    target_id: str = "",
    *,
    ...
    try:
        ...
        record_operation_audit(
            ...
            request_ip=str(getattr(web.ctx, "ip", "") or ""),
            user_agent=str(env.get("HTTP_USER_AGENT", "") or ""),
            detail=detail or {},
        )
...
class AssetsHandler:
    def GET(self, file_path):  # 修改默认参数
        try:
            ...
            if file_path in ('js/console.js', 'css/console.css'):
                web.header('Cache-Control', 'no-cache, no-store, must-revalidate')
                web.header('Pragma', 'no-cache')
...
class InvestmentUserStatusHandler:
    def POST(self, openid, action):
        ...
        try:
            ...
            before_state = _investment_customer_snapshot(before_user)
            if action == "enable":
                enable_user(openid, actor=admin)
            else:
                disable_user(openid, actor=admin)
            after_state = _investment_customer_snapshot(get_user_by_openid(openid))
            _record_investment_operation(
                f"customer.{action}",
                "customer",
...
class InvestmentDailyContentInvalidateHandler:
    def POST(self, content_id):
        admin = _require_investment_permission("content.publish")
        try:
            ...
            logger.error(f"[Investment] content invalidate error: {e}")
            return _investment_json_response({"status": "error", "message": str(e)})


class InvestmentDailyContentExpiryHandler:
    def PATCH(self, content_id):
        ...
        try:
            ...
            return _investment_json_response({"status": "success", "expires_at": expires_at})
        except Exception as e:
            logger.error(f"[Investment] content expiry update error: {e}")
            return _investment_json_response({"status": "error", "message": str(e)})
...
class InvestmentContentRecordsHandler:
    def GET(self):
        ...
        try:
            ...
            page, page_size = _investment_safe_pagination(params, 80)
            record_status = getattr(params, "status", "") or ""
            keyword = getattr(params, "keyword", "") or ""
            start_date = _investment_date_bound(getattr(params, "start_date", ""))
            end_date = _investment_date_bound(getattr(params, "end_date", ""), end=True)
...
class InvestmentArtifactPackagesHandler:
    def GET(self):
        _require_investment_permission("content.read")
        try:
            ...
            return _investment_json_response({
                "status": "success",
                "packages": packages,
                "tree": build_artifact_package_tree(packages),
                "pagination": _investment_pagination_payload(page, page_size, total),
            })
        except Exception as e:
...
class InvestmentSkillPackageUploadHandler:
    def POST(self):
        ...
        try:
            ...
            _record_investment_operation("skill.upload", "investment_skill", uploaded.get("skill_key", ""), admin=admin, detail={"filename": os.path.basename(filename), "package": True})
            return _investment_json_response({
                "status": "success",
                "uploaded": uploaded,
                "skills": list_all_skills(),
...
class InvestmentSkillUploadHandler:
    def POST(self, skill_key):
        ...
        try:
            ...
            _record_investment_operation("skill.upload", "investment_skill", skill_key, admin=admin, detail={"filename": os.path.basename(filename), "version_id": version.get("version_id", "")})
            return _investment_json_response({
                "status": "success",
                "version": version,
                "skills": list_all_skills(),
